...
def main_worker(gpu, ngpus_per_node, args):
	...
	# create optimizer
	optimizer = torch.optim.SGD(trainer.parameters(), args.lr, weight_decay=4e-5)
	...
	for epoch in range(args.start_epoch, args.epochs):		
		...
		if (epoch + 1) % 5 == 0:  # calculate on training set			
			if args.pretrain_vae:
				loss, recon_loss, kld_normal, kld_laplace = trainer.training_step(True)
				acc1_reduced = 0.0  # No accuracy when pretraining VAE
				acc5_reduced = 0.0  # No accuracy when pretraining VAE
				class_losses_reduced = 0.0  # No class loss when pretraining VAE
			else:
			...
			if args.pretrain_vae:
				loss, recon_loss, kld_normal, kld_laplace = trainer.training_step(False)
				losses_reduced = loss.cuda().item()				
				class_losses_reduced = 0.0  # No class loss when pretraining VAE
			else:
		...
		if ((epoch + 1) % (epoch + 1) == 0) and args.evaluate:  # or epoch > 18
			if args.pretrain_vae:
				loss, recon_loss, kld_normal, kld_laplace = trainer.validate()
				losses_reduced = loss.cuda().item()
				acc1_reduced = 0.0  # No accuracy when pretraining VAE
				acc5_reduced = 0.0  # No accuracy when pretraining VAE
				class_losses_reduced = 0.0  # No class loss when pretraining VAE
