...
from model.causal import MLP, BetaVAE_MLP, NPChangeTransitionPrior
...
class Trainer(nn.Module):
	def __init__(
		...
		self.cls_net = MLP(input_dim=self.seq_len*(z_dim*2), hidden_dim=128, output_dim=self.action_dim, num_layers=3)
	...
	def training_step(self, if_calculate_acc):
		...
		for batch in self.train_loader:
			...
			if not self.pretrain_vae:
				...
				#print("Training Loss: ", loss.item(), " Class Loss: ", class_loss.item(), " Recon Loss: ", recon_loss.item(), " KLD Normal: ", kld_normal.item(), " KLD Laplace: ", kld_laplace.item())
			else:
			...
			# Compute accuracy only when requested and a classifier prediction exists
			if if_calculate_acc and (not self.pretrain_vae):
				with torch.no_grad():				
	...
	def validate(self):
		"""
		This is for test set, lavila uses 30 crops at test time. So dimension is [batch_size, num_of_crops, temporal, feature]
		"""
		losses = AverageMeter()
		recon_losses = AverageMeter()
		kld_normal_losses = AverageMeter()
